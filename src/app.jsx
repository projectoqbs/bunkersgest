...
const NAV_META = {
  ...
  tanques_p1:       { label:"Planta 1 — QBS002", icon:"🚢" },
  tanques_qbs003:   { label:"Planta 1 — QBS003", icon:"🚢" },
...
// Retorna VCF; galones_guia_neto = galones_guia_brutos × VCF
...
export default function App() {
  ...
  const [trazColsDesc, setTrazColsDesc] = useState(new Set(["placa","fecha","cmt","producto","transportadora","guia","tiquete","api","glsGuia","glsBas","glsGuiaNeto","varPct","pbs","ot","tanques","horaInicio","horaFinal","duracion","pesoIng","pesoSal","pesoNeto","rpm","presion"]));
...
async function calcularGalones(tanque, ullage, temp, api, esDespues, index) {
  ...
  async function guardarCMT(e) {
    ...
    const carrosConGls = cmtCarros.map(carro => {
      ...
      // Para MGO: persistir galones_guia_neto = galones_guia (brutos) × VCF del tiquete de laboratorio
    ...
    if (form.id) {
      ...
      if (sedeActual === "MALAMBO") {
        ...
        if (tanqueOrigen) {
          ...
          else if (tanqueOrigen.startsWith("QBS002-") || tanqueOrigen.startsWith("QBS003-") || tanqueOrigen.startsWith("TKT-")) plantaActual = "PLANTA 1";
        }
      }
...
const puedeEditar = (modulo, creado_por, created_at) => {
  ...
  const ALL_MODULOS = ["dashboard","viajes","tiquetes","pbs","tanques","despacho","trazabilidad","programacion","usuarios","liquidador_p1"];
        ...
        <motion.div key={nav}
                        ...
                        {muestras.map(({ot:o,tr,tq,yaAnalizado,horasRecirc},idx)=>(
                          <div key={`${o.id}-${tr.destino}-${idx}`} style={{background:yaAnalizado?`${T.success}10`:`${T.orange}10`,border:`1px solid ${yaAnalizado?T.success:T.orange}44`,borderRadius:8,padding:"8px 14px",fontSize:12}}>
                            ...
                            {yaAnalizado
                              ...
                              : <button onClick={()=>{
                                  const esP1 = (tr.destino||"").startsWith("QBS002-") || (tr.destino||"").startsWith("QBS003-") || (tr.destino||"").startsWith("TKT-");
                                        ...
                                        {(c.carros||[]).map((cr,i)=>(
                                          <div key={i} style={{background:"#f8fafc",borderRadius:8,padding:"8px 10px",fontSize:11,border:`1px solid ${T.border}`}}>
                                            ...
                                            {(()=>{
                                              ...
                                              const glsGuiaCr = Number(cr.galones_guia||0);
                                              const glsGuiaNeto = cr.galones_guia_neto || ((vcf&&glsGuiaCr)?Math.round(glsGuiaCr*vcf):null);
                                              return glsGuiaNeto ? (
                                                <div style={{marginTop:4,padding:"4px 6px",background:`${T.success}15`,borderRadius:4,borderLeft:`2px solid ${T.success}`}}>
                                                  <div style={{color:T.success,fontWeight:700}}>Gls Guía Neto: {fmt(glsGuiaNeto)}</div>
          ...
          {nav==="tanques_qbs003" && (()=>{
            ...
            const TanqueQ3 = ({ id }) => {
              const t      = byId3(id);
              const nivel  = Number(t.nivel||0);
              const cap    = Number(t.capacidad || CAP_QBS003[id] || 40000);
              const pct    = cap > 0 ? Math.min(100, Math.round((nivel/cap)*100)) : 0;
              const color  = getProductColor(t.producto);
              ...
              return (
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, flex:1, minWidth:0 }}>
                  <div
                    ...
                    {pct > 0 && pct < 100 && (
                      <div style={{ position:"absolute", left:0, right:0, bottom:`${pct}%`,
                        height:2, background:"rgba(255,255,255,0.6)", pointerEvents:"none" }}/>
                    )}
                    <div style={{ position:"absolute", top:4, left:0, right:0, textAlign:"center" }}>
                      <span style={{ fontSize:11, fontWeight:900, color:"#0077CC",
                        textShadow:"0 1px 2px rgba(0,0,0,0.5)", fontFamily:"monospace" }}>{pct}%</span>
                    </div>
                    <div style={{ position:"absolute", bottom:6, left:2, right:2, textAlign:"center" }}>
                      {editando ? (
                        <input autoFocus value={tankProdEdit.val}
                          onChange={e => setTankProdEdit({ id, val:e.target.value })}
                          onKeyDown={e=>{ if(e.key==="Enter") guardarProductoTanque3(id, tankProdEdit.val); if(e.key==="Escape") setTankProdEdit(null); }}
                          onBlur={() => guardarProductoTanque3(id, tankProdEdit.val)}
                          style={{ background:"rgba(0,0,0,0.65)", border:`1px solid ${T.orange}`, borderRadius:3,
                            color:"#fff", fontSize:9, fontWeight:700, padding:"1px 4px", width:"90%",
                            textTransform:"uppercase", outline:"none", textAlign:"center" }}/>
                      ) : (
            ...
            const totalNivel3 = [...BABOR,...ESTRIBOR].reduce((a,id)=>a+Number(byId3(id).nivel||0),0);
            const totalLibre3 = [...BABOR,...ESTRIBOR].reduce((a,id)=>{ const t=byId3(id); return a+Math.max(0,capOp3(t)-Number(t.nivel||0)); },0);
            const BAND_E = "#1e6fa8";
            const BAND_B = "#0d3d5c";
            ...
            return (
              <div style={{ padding:"16px 20px" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
                  <div>
                    <div style={{ fontWeight:900, fontSize:20, color:T.navy, display:"flex", alignItems:"center", gap:8 }}><Ship size={20}/>Planta 1 — Barcaza QBS003</div>
                    <div style={{ fontSize:11, color:T.muted }}>12 tanques · Estribor (ER) · Babor (BR) · doble-clic para editar producto</div>
                  </div>
                  <div style={{ display:"flex", gap:10 }}>
                    <div style={{ background:T.navy, borderRadius:10, padding:"8px 16px", textAlign:"center" }}>
                      <div style={{ fontSize:9, color:"rgba(255,255,255,0.55)", fontWeight:700, letterSpacing:1.2, textTransform:"uppercase" }}>Stock</div>
                      <div style={{ fontSize:16, fontWeight:900, color:T.orange, fontFamily:"monospace" }}>{fmt(totalNivel3)} gls</div>
                    </div>
                    <div style={{ background:"#0d3d5c", borderRadius:10, padding:"8px 16px", textAlign:"center" }}>
                      <div style={{ fontSize:9, color:"rgba(255,255,255,0.55)", fontWeight:700, letterSpacing:1.2, textTransform:"uppercase" }}>Libre</div>
                      <div style={{ fontSize:16, fontWeight:900, color:"#6ee7b7", fontFamily:"monospace" }}>{fmt(totalLibre3)} gls</div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ textAlign:"center", padding:"10px 0 6px",
                    borderBottom:"2px solid #1a4a6e", marginBottom:4 }}>
                    <span style={{ fontWeight:900, fontSize:13, color:"#7ec8e3",
                      letterSpacing:4, textTransform:"uppercase", fontFamily:"monospace", display:"flex", alignItems:"center", gap:5, justifyContent:"center" }}><Anchor size={13}/>QBS003</span>
                  </div>

                  {/* ESTRIBOR (ER) */}
                  <div style={{ padding:"10px 16px 6px" }}>
                    <div style={{ display:"flex", alignItems:"center", marginBottom:8 }}>
                      <div style={{ width:10, alignSelf:"stretch", background:BAND_E, borderRadius:3, marginRight:8, minHeight:16 }}/>
                      <span style={{ fontWeight:800, fontSize:10, color:"#7ec8e3", letterSpacing:2, textTransform:"uppercase" }}>Estribor (ER)</span>
                      <span style={{ marginLeft:"auto", fontSize:10, color:"rgba(255,255,255,0.5)", fontFamily:"monospace" }}>{fmt(ESTRIBOR.reduce((a,id)=>a+Number(byId3(id).nivel||0),0))} gls</span>
                    </div>
                    <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
                      {ESTRIBOR.map(id => <TanqueQ3 key={id} id={id}/>)}
                    </div>
                  </div>

                  <div style={{ height:2, background:"linear-gradient(90deg,transparent,#1a4a6e 20%,#2a6a9e 50%,#1a4a6e 80%,transparent)", margin:"8px 16px" }}/>

                  {/* BABOR (BR) */}
                  <div style={{ padding:"6px 16px 0" }}>
                    <div style={{ display:"flex", alignItems:"center", marginBottom:8 }}>
                      <div style={{ width:10, alignSelf:"stretch", background:BAND_B, borderRadius:3, marginRight:8, minHeight:16 }}/>
                      <span style={{ fontWeight:800, fontSize:10, color:"#93c5fd", letterSpacing:2, textTransform:"uppercase" }}>Babor (BR)</span>
                      <span style={{ marginLeft:"auto", fontSize:10, color:"rgba(255,255,255,0.5)", fontFamily:"monospace" }}>{fmt(BABOR.reduce((a,id)=>a+Number(byId3(id).nivel||0),0))} gls</span>
                    </div>
                    <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
                      {BABOR.map(id => <TanqueQ3 key={id} id={id}/>)}
                    </div>
                  </div>
            ...
            const filasDescargue = (() => {
              ...
              (cmts||[]).filter(c=>(c.tipo_operacion||"")==="DESCARGUE DE CARROTANQUE").forEach(cm => {
                ...
                (cm.carros||[]).forEach(cr => {
                  ...
                  // Para MGO: usar galones_guia_neto persistido o recalcular con VCF
                  const glsGuiaNeto = esMGORow ? (Number(cr.galones_guia_neto||0) || (()=>{
                    const vcfT = Number(tq?.factor_conversion||0)||calcVCF(Number(tq?.api_corregido||0),Number(tq?.temp_observada||0));
                    const glsGuia = Number(cr.galones_guia||0);
                    return (vcfT&&glsGuia)?Math.round(glsGuia*vcfT):0;
                  })()) : 0;
                  rows.push({cm,cr,tq,pb,ot,viaje,tanquesDesc,glsBas,glsGuiaNeto,esMGORow});
                });
              });
              return rows;
            })();
                      ...
                      {trazColPanel && (()=>{
                        const colsDef = trazEstado==="DESCARGUE"
                          ? [
                              ...
                              {k:"api",l:"API"},{k:"glsGuia",l:"Gls Guía (Bruto)"},{k:"glsBas",l:"Gls Báscula (Neto)"},{k:"glsGuiaNeto",l:"Gls Guía (Neto)"},{k:"varPct",l:"Variación %"},
                  ...
                  <div data-scroll-key="trazabilidad-descargue" style={{overflow:"auto",maxHeight:"calc(100vh - 310px)",borderRadius:10,border:`1px solid ${T.border}`}}>
                    <table style={{width:"100%",borderCollapse:"collapse",background:T.card,minWidth:400}}>
                      <thead><tr>
                        ...
                        {C.has("glsGuia")        && <th style={thStyle}>Gls Guía (Bruto)</th>}
                        {C.has("glsBas")         && <th style={thStyle}>Gls Báscula (Neto)</th>}
                        {C.has("glsGuiaNeto")    && <th style={thStyle}>Gls Guía (Neto)</th>}
                      ...
                      <tbody>
                        ...
                        {descFiltradas.map(({cm,cr,tq,pb,ot,viaje,tanquesDesc,glsBas,glsGuiaNeto,esMGORow},i)=>{
                          ...
                          <tr key={`${cm.id}-${cr.placa}-${i}`} style={{background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>
                            ...
                            {C.has("glsGuiaNeto")    && <td style={tdS({fontWeight:700,color:glsGuiaNeto>0?T.success:T.muted})}>
                              {glsGuiaNeto>0?fmt(glsGuiaNeto):(esMGORow?"—":"n/a")}
                            </td>}
                            {C.has("varPct")         && (()=>{
                              const gGuia=Number(cr.galones_guia||viaje?.volumen_guia||0);
                              // Variación: báscula neto vs guía neto (MGO) o guía bruto (otros, ya son netos)
                              const guiaRef = esMGORow&&glsGuiaNeto>0 ? glsGuiaNeto : gGuia;
                              const pct=guiaRef>0&&glsBas>0?((glsBas-guiaRef)/guiaRef*100):null;
              ...
              <div style={{width:160,flexShrink:0}}>
                {["ENTREGA A MOTONAVE","ENTREGA A CARROTANQUE"].includes(form.tipo_operacion||"") ? (
                  <Sel label="Barcaza" value={form.planta||""} onChange={e=>{setForm(prev=>({...prev,planta:e.target.value}));}}>
                    ...
                    <option value="QBS003">QBS003</option>
            ...
            {(()=>{
              ...
              const tankEnPlanta = (t, p) => {
                ...
                if (esEntregaMot) {
                  ...
                  if (p==="QBS003") return t.id.startsWith("QBS003-");
                  if (p==="TANQUES TIERRA") return t.id.startsWith("TKT-");
                  return t.id.startsWith("QBS002-")||t.id.startsWith("QBS003-")||t.id.startsWith("TKT-");
                }
                if (p==="PLANTA 1") return t.id.startsWith("QBS002-") || t.id.startsWith("QBS003-") || t.id.startsWith("TKT-");
                      ...
                      <select value={rec.tanque} onChange={e=>{const n=[...cmtRecepcion];n[i]={...n[i],tanque:e.target.value};setCmtRecepcion(n);}} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:6,padding:"6px 10px",color:T.text,fontSize:13,fontFamily:"system-ui,sans-serif",outline:"none"}}>
                        <option value="">—</option>{(()=>{
                          ...
                          if(esEntrega){
                            ...
                            if(p==="QBS003") return tanquesEnriquecidos2.filter(t=>t.id.startsWith("QBS003-"));
                            if(p==="TANQUES TIERRA") return tanquesEnriquecidos2.filter(t=>t.id.startsWith("TKT-"));
                            return tanquesEnriquecidos2.filter(t=>t.id.startsWith("QBS002-")||t.id.startsWith("QBS003-")||t.id.startsWith("TKT-"));
                          }
                          if(p==="PLANTA 1") return tanquesEnriquecidos2.filter(t=>t.id.startsWith("QBS002-")||t.id.startsWith("QBS003-")||t.id.startsWith("TKT-"));
                          if(p==="PLANTA 2") return tanquesEnriquecidos2.filter(t=>t.id.startsWith("TK-"));
                          return tanquesEnriquecidos2.filter(t=>t.id.startsWith("TK-")||t.id.startsWith("QBS002-")||t.id.startsWith("QBS003-")||t.id.startsWith("TKT-"));
          ...
          {(form.tipo_operacion||"")==="DESCARGUE DE CARROTANQUE" && <><div ref={cmtCarrosRef}/><Section title="Carros Descargados" color={T.muted}>
            ...
            {cmtCarros.map((carro,i)=>(
              <div key={i} style={{background:T.bg,borderRadius:8,padding:"12px 14px",marginBottom:12,border:`1px solid ${T.border}`}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)",gap:8,alignItems:"end"}}>
                ...
                {(()=>{
                  ...
                  const apiLab  = Number(tq?.api_corregido||0);
                  const tempLab = Number(tq?.temp_observada||0);
                  // VCF: usar factor_conversion del tiquete si existe, si no recalcular
                  const vcf = Number(tq?.factor_conversion||0) || calcVCF(apiLab, tempLab);
                  // Gls Guía Neto = galones_guia (brutos) × VCF
                  const glsGuia = Number(carro.galones_guia||0);
                  const glsGuiaNeto = (vcf&&glsGuia) ? Math.round(glsGuia*vcf) : null;
                  ...
                  return (
                    <div style={{marginTop:8,padding:"10px 12px",background:`${T.orange}12`,border:`1px solid ${T.orange}44`,borderRadius:6}}>
                      <div style={{fontSize:10,fontWeight:700,color:T.orange,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>Conversión Galones Netos Guía (ASTM D1250)</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,alignItems:"end"}}>
                        ...
                        <div><Lbl>Gls Guía Neto</Lbl><div style={{background:`${T.success}22`,border:`1.5px solid ${T.success}88`,borderRadius:6,padding:"10px 12px",fontSize:13,fontFamily:"monospace",color:glsGuiaNeto?T.success:"#aab4be",fontWeight:700,minHeight:43,display:"flex",alignItems:"center"}}>{glsGuiaNeto?fmt(glsGuiaNeto):"—"}</div></div>
                      </div>
          ...
          {(form.tipo_operacion||"")==="PORTEO" && (()=>{
            ...
            const renderTanqueSection = (color, planta, setPlanta, rows, setRows, isDescarga=false) => {
              const tanquesPlanta = planta==="PLANTA 1"
                ? tanquesPorteoAll.filter(t=>t.id.startsWith("QBS002-")||t.id.startsWith("QBS003-")||t.id.startsWith("TKT-"))
                : planta==="PLANTA 2"
